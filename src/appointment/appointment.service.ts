...
console.log(AdminPatientMobileNo.length,"AdminPatientMobileNo.length");
 ...
 console.log(AdminTrimmedMobileno,"////////");}
  ...
  const buffered_ip =  Buffer.from(
    hosCredentials[0].ip,
    'base64'
  );
  const buffered_db_name = Buffer.from(
    hosCredentials[0].db_name,
    'base64'
  );
  const buffered_db_password = Buffer.from(
    hosCredentials[0].db_password,
    'base64'
  );
  const buffered_db_user = Buffer.from(
    hosCredentials[0].username,
    'base64'
  );
...
console.log("staffEmailAdmin",staffEmailAdmin.email);
 ...
 let HOSStaffId = HosStaff.id
  ...
  var HOStransaction_id:number
    ...
    const HOScharge = await dynamicConnection.query('select charge_id from shift_details where shift_details.staff_id = ?',
    [HOSStaffId])
...
const Patient_charges_insert = await dynamicConnection.query(
...
// console.log("12341234",hos_appointment_id);
...
try {
  const HOSbookAppnt = await dynamicConnection.query(
...
GROUP BY
    doctor_id, doctor_name, hospital_id, hospital_name, date, time, appointment_status, patient_id,appointment_id;`,
     [patient_id]
     )
        return pastAppoint;
      }
...
async  findDoctors(speciality:string,search:string) {
    let query = `SELECT 
...
async  update(id: number, AppointmentEntity: Appointment) {
  ...
  try {
    ...
    // console.log(hosCredentials,"hosCredentials");
    ...
    // console.log("buffered_ip",buffered_ip);
    
    const decryptedIP = this.decrypt(buffered_ip, this.privateKey);
    const decryptedDB_NAME = this.decrypt(buffered_db_name, this.privateKey);
    const decryptedDB_PASS = this.decrypt(buffered_db_password, this.privateKey);
    const decryptedDB_USER = this.decrypt(buffered_db_user, this.privateKey);
    // console.log(decryptedIP,"decryptedIP");
